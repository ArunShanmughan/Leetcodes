...
// The ith keypress had a duration of releaseTimes[i] - releaseTimes[i - 1], and the 
...
// these multiple presses of the same key may not have had the same duration.

// Return the key of the keypress that had the longest duration. If there are multiple 
// such keypresses, return the lexicographically largest key of the keypresses.

var releaseTimes = [9,29,49,50], keysPressed = "cbcd";

var slowestKey = function (releaseTimes, keysPressed) {
  var num = releaseTimes[0];
  var str = keysPressed[0];

  for (let i = 1; i < releaseTimes.length; i++) {
      let res = releaseTimes[i] - releaseTimes[i - 1];

      if (res > num || (res === num && keysPressed[i] > str)) {
          num = res;
          str = keysPressed[i];
      }
  }
